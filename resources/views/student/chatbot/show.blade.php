...
@section('title', 'Chat - ' . $conversation->title)
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-3 border-t border-gray-200">
                        <a href="{{ route('student.chatbot.create') }}" class="block w-full text-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors">
                            + Percakapan Baru
                        </a>
                    </div>
                </div>
            ...
            <div class="lg:col-span-3">
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 chat-container">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-white/80">AI Assistant Karsa</p>
                                </div>
                            </div>
                            <div class="flex items-center space-x-2">
                                <button onclick="archiveConversation({{ $conversation->id }})" class="p-2 text-white/80 hover:text-white hover:bg-white/10 rounded-lg transition-colors">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"></path>
                                    </svg>
                                </button>
                                <button onclick="deleteConversation({{ $conversation->id }})" class="p-2 text-white/80 hover:text-white hover:bg-white/10 rounded-lg transition-colors">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Messages Area -->
                    <div class="messages-container p-6 bg-gray-50" id="messagesContainer">
                            ...
                            <div class="message-bubble {{ $message->isUser() ? 'user-message' : 'assistant-message' }} px-4 py-3 shadow-sm">
                                ...
                                <div class="mt-3 pt-3 border-t {{ $message->isUser() ? 'border-white/20' : 'border-gray-300' }}">
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <div class="source-badge text-xs {{ $message->isUser() ? 'text-white/90' : 'text-gray-700' }}">
                                            • {{ $ref->document->title }}
                                        </div>
                                        @endforeach
                                    </div>
                    ...
                    <div class="px-6 py-4 border-t border-gray-200 bg-white">
                        <form id="chatForm" class="flex items-end space-x-3">
                            ...
                            <button
                                type="submit"
                                id="sendButton"
                                class="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg shadow-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
                            >
                                <span>Kirim</span>
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    function addMessageToUI(role, content, sources = []) {
        ...
        if (role === 'assistant' && sources && sources.length > 0) {
            ...
            sources.forEach(source => {
                ...
                sourceItem.className = 'source-badge text-xs text-gray-700';
