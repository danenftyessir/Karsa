...
<a href="{{ route('student.browse-problems.show', $problem->id) }}"
   class="problem-card-list group bg-white rounded-xl shadow-sm hover:shadow-xl border border-gray-100 overflow-hidden transition-all duration-300 hover:-translate-y-1 block">
    ...
    <div class="flex gap-6 p-6">
        ...
        <div class="w-48 h-32 bg-gradient-to-br from-blue-100 to-blue-50 rounded-lg overflow-hidden flex-shrink-0 relative">
            @if($problem->images && $problem->images->count() > 0)
                @php
                    ...
                    $imageUrl = supabase_url($imagePath);
                @endphp
                <img src="{{ $imageUrl }}"
                     alt="{{ $problem->title }}"
        ...
        <div class="flex-1 min-w-0 flex flex-col">
            ...
            {{-- sdg categories - sama dengan grid (max 3) --}}
            ...
            {{-- stats --}}
            <div class="flex items-center gap-6 text-xs text-gray-600 mb-4 pb-4 border-b border-gray-100">
            ...
            {{-- footer dengan deadline, difficulty, dan action button --}}
            <div class="flex items-center justify-between mt-auto">
                ...
                <button class="px-6 py-2.5 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white font-bold rounded-lg transition-all shadow-md hover:shadow-lg">
                    Lihat Detail Proyek
                </button>
            </div>
        </div>
